...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        grad1 = np.ones_like(self.history[1].arr)
        grad2 = np.ones_like(self.history[2].arr)
    ...
    def grad_matmul(self, gradients=None):
        ...
        grad1 = gradients @ self.history[2].arr.T
        grad2 = self.history[1].arr.T @ gradients
    ...
    def backward(self, gradients=None):
        ...
        Setting the gradient of leaf node(Tensor) wrt to the node(Tensor) the backward in called on.
        Ex:
            a = Tensor(..) #leaf
            b = Tensor(..) #leaf
            c = a+b
            c.backward()
            computes:
                dc/da -> Store in a if a requires_grad
                dc/db -> Store in b if b requires_grad
        Args:
        ...
        if self.requires_grad:
            if self.history[0] == 'leaf' and self.requires_grad:
                if gradients is not None:
                    self.grad += gradients
                else:
