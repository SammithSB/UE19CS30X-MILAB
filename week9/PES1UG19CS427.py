...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        for i in range(data.shape[0]):
            cluster_assign[i] = np.argmin(
                np.linalg.norm(data[i] - self.centroids, axis=1))
    ...
    def m_step(self, data, cluster_assgn):
        ...
        for i in range(self.n_cluster):
            # find mean of all the samples in the cluster
            new_centroid = np.mean(data[cluster_assgn == i], axis=0)
    ...
    def evaluate(self, data, cluster_assign):
        ...
        # find objective using 2 loops
        metric = 0
        for i in range(self.n_cluster):
                metric += np.linalg.norm(data[cluster_assign == i] - self.centroids[i]) ** 2
